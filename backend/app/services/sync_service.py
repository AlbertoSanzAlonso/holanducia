...
from backend.app.services.embedding_service import EMBEDDING_MODEL, EmbeddingService
...
class SyncService:
    ...
    async def get_database_stats(self) -> dict:
        ...
        total_embedded = await self.db.scalar(
        ...
        if running:
            sync_in_progress = {
                ...
                "sources": running.sources or [],
            }
        if last_completed:
            last_sync = {
                "id": last_completed.id,
                "started_at": last_completed.started_at.isoformat() if last_completed.started_at else None,
                ...
                "sources": last_completed.sources or [],
            }
    ...
    async def cleanup_stuck_runs(self, *, max_age_hours: int = 2) -> int:
        ...
        for run in stuck:
            run.status = "failed"
            run.completed_at = now
            run.stats = {**(run.stats or {}), "error": "timeout — sync atascado, cerrado al arrancar API"}
        if stuck:
            await self.db.commit()
        return len(stuck)
